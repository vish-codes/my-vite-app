...
const GeneratePDF = ({ invoiceData }) => {
  ...
  const commonDataForPdf = {
    ...
    dateOfInvoice: invoice?.issue_date ? new Date(invoice.issue_date).toLocaleDateString() : "N/A",
  ...
  const resourcesArr = [
    {
      ...
      days: invoice?.days || 0,
      hours: 8,
      payPerDay: invoice?.total_amount ? Math.round(Number(invoice.total_amount) / (invoice.days || 1)) : 0,
    },
  ]

  const calculateTotals = (gstRate) => {
    const subTotal = resourcesArr.reduce((acc, val) => acc + val.payPerDay * val.days, 0)
  ...
  const generatePDF = (status) => {
    ...
    resourcesArr.forEach((res, index) => {
      ...
      doc.setFont("helvetica", "normal")
      doc.setFontSize(7.5)
      doc.text(`Consultancy charges ${res.employeeName} on ${res.workingOn} (${res.userId})`, 31.5, currentY)
      ...
      {pdfUrl ? (
        <div className="bg-white shadow-lg flex flex-col rounded-lg overflow-hidden">
          ...
          <div className="relative w-full" style={{ height: "600px" }}>
            <iframe src={pdfUrl} style={{ width: "100%", height: "100%" }} frameBorder="0" title="PDF Preview" />
          </div>
        </div>
      ) : (
        <div className="text-center py-12 text-slate-600">Loading invoice preview...</div>
      )}
    </div>
  )
}
